---
layout: post
title: CSAW'23 Qualifiers unlimited_subway Write-Up 
excerpt: "My write up for the pwn challenge unlimited_subway on CSAW'23 Qualifiers."
categories: [Writeups]
tags: [pwn]
---
# CSAW'23 Qualifiers unlimited_subway Write-Up

At the beginning of the CTF there were issues with the GMU Wi-Fis ability to access the remotes which unfortunately prevented us from doing as well as we hoped to this CTF. However, we were able to solve a few challenges, one of which was my favorite:  `unlimited_subway` created by `079`. 
...
Now we can overflow the buffer and overwrite the return address with the address of `print_flag()`. We can do this using the name size and name inputs. But my original script had an issue, negative indexing was causing the program to segfault. Instead of debugging the issue, we can just use positive indexing to read the canary. 
